...
function openMapAndScale() {
    ...
    const img = modal.querySelector('img[usemap]');
    if (!img) return;

    // 定义一个可重用的坐标缩放函数
    const scaleCoords = () => {
        // 必须在图片加载完成后才能获取到 naturalWidth
        if (!img.complete || img.naturalWidth === 0) return;

        // 计算缩放比例 = 图片当前显示的宽度 / 图片的原始宽度
    ...
    // 如果图片已经加载完成，立即缩放坐标；否则，等待加载完成后再执行
    if (img.complete) {
        scaleCoords();
    } else {
        img.onload = scaleCoords;
    }

    // 监听窗口大小变化，实时重算坐标（例如手机横竖屏切换）
    window.addEventListener('resize', scaleCoords);
    
    // 关闭弹窗时，移除resize监听器，避免内存泄漏
    modal.querySelector('.close').addEventListener('click', () => {
        window.removeEventListener('resize', scaleCoords);
    }, { once: true }); // once: true 确保这个关闭监听器只添加一次
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (editMapBtn && editMapModal && editMapCloseBtn) {
        ...
        // 【修改】点击按钮时，调用新的全屏打开和缩放逻辑
        editMapBtn.addEventListener('click', () => {
            const img = editMapModal.querySelector('img[usemap]');
            
            const scaleEditMapCoords = () => {
                if (!img.complete || img.naturalWidth === 0) return;
                const scaleFactor = img.clientWidth / img.naturalWidth;
                const map = editMapModal.querySelector(`map[name="${img.useMap.substring(1)}"]`);
                if (!map) return;
            ...
            if (img.complete) {
                ...
                img.onload = scaleEditMapCoords;
            }
            
            window.addEventListener('resize', scaleEditMapCoords);
            editMapCloseBtn.addEventListener('click', () => {
                window.removeEventListener('resize', scaleEditMapCoords);
            }, { once: true });
        });
